...
import com.limelight.gyro.GyroAimManager;
import com.limelight.gyro.GyroSettingsActivity;
import com.limelight.nvstream.NvConnection;
import com.limelight.touch.TouchGamepadConfig;
import com.limelight.touch.TouchGamepadOverlay;
...
public class Game extends Activity implements SurfaceHolder.Callback,
        ...
        TouchGamepadOverlay.InputInjector {
    ...
    private static final int FOUR_FINGER_TAP_THRESHOLD = 400;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        // 初始化陀螺仪瞄准管理器
        gyroAimManager = new GyroAimManager(this, (deltaX, deltaY) -> {
        ...
        // 添加虚拟触控手柄覆盖层（手游 FPS 风格摇杆+按钮+触屏瞄准）
        ...
        if (prefConfig.lowLatencyMode) {
            getWindow().addFlags(WindowManager.LayoutParams.FLAG_KEEP_SCREEN_ON);
            displayTransientMessage("三角洲低延迟模式已启用");
        }
    ...
    private float prepareDisplayForRendering() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            ...
            if (display.getMode().getModeId() != bestMode.getModeId()) {
                // Some TVs (notably Chromecast 4K) can select a bad lower-resolution mode when
                // preferredDisplayModeId is used for a refresh-only switch. Keep the setFrameRate()
                // workaround for TVs, but request the physical mode explicitly on phones/tablets.
                // Several mobile OEMs accept setFrameRate() without actually switching the panel,
                // which produces uneven cadence such as 90 FPS content on a 144 Hz display.
                boolean isTelevision = getPackageManager().hasSystemFeature(PackageManager.FEATURE_TELEVISION) ||
                ...
                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S ||
                    ...
                    // Some OEM display policies ignore the mode ID but still honor the legacy
                    // refresh-rate hint, so publish both when we need an exact cadence match.
    ...
    protected void onResume() {
        ...
        // 从陀螺仪设置页返回后，重新加载设置并应用到管理器
        if (gyroAimManager != null) {
            gyroAimManager.setSettings(com.limelight.gyro.GyroAimSettings.load(this));
            // onPause 会 stop 传感器，返回后若连接仍在需重新 start
            if (connected && gyroAimManager.isGyroAvailable()) {
                gyroAimManager.start();
            }
        }

        // 重新加载触控手柄配置（编辑模式返回后生效）
        if (touchOverlay != null) {
            touchOverlay.setConfig(TouchGamepadConfig.load(this));
        }
    }
    ...
    public boolean handleKeyDown(KeyEvent event) {
        ...
        if ((eventSource == InputDevice.SOURCE_MOUSE ||
            ...
            if (!prefConfig.mouseNavButtons) {
                doMouseDown(MouseButtonPacket.BUTTON_RIGHT);
            }
    ...
    public boolean handleKeyUp(KeyEvent event) {
        ...
        if ((eventSource == InputDevice.SOURCE_MOUSE ||
            ...
            // care of this.
            if (!prefConfig.mouseNavButtons) {
                doMouseUp(MouseButtonPacket.BUTTON_RIGHT);
            }
                    ...
                    eventSource == 12290) // 12290 = Samsung DeX mode desktop mouse
            {
                ...
                if ((changedButtons & MotionEvent.BUTTON_PRIMARY) != 0) {
                    if ((buttonState & MotionEvent.BUTTON_PRIMARY) != 0) {
                        doMouseDown(MouseButtonPacket.BUTTON_LEFT);
                    }
                    else {
                        doMouseUp(MouseButtonPacket.BUTTON_LEFT);
                    }
                }
                ...
                if ((changedButtons & (MotionEvent.BUTTON_TERTIARY | MotionEvent.BUTTON_STYLUS_SECONDARY)) != 0) {
                    if ((buttonState & (MotionEvent.BUTTON_TERTIARY | MotionEvent.BUTTON_STYLUS_SECONDARY)) != 0) {
                        doMouseDown(MouseButtonPacket.BUTTON_MIDDLE);
                    }
                    else {
                        doMouseUp(MouseButtonPacket.BUTTON_MIDDLE);
                    }
                }
                ...
                if (prefConfig.mouseNavButtons) {
                    if ((changedButtons & MotionEvent.BUTTON_BACK) != 0) {
                        if ((buttonState & MotionEvent.BUTTON_BACK) != 0) {
                            doMouseDown(MouseButtonPacket.BUTTON_X1);
                        }
                        else {
                            doMouseUp(MouseButtonPacket.BUTTON_X1);
                        }
                    }

                    if ((changedButtons & MotionEvent.BUTTON_FORWARD) != 0) {
                        if ((buttonState & MotionEvent.BUTTON_FORWARD) != 0) {
                            doMouseDown(MouseButtonPacket.BUTTON_X2);
                        }
                        else {
                            doMouseUp(MouseButtonPacket.BUTTON_X2);
                        }
                    }
                }
    ...
    public void mouseButtonEvent(int buttonId, boolean down) {
        ...
        if (down) {
            doMouseDown(buttonIndex);
        }
        else {
            doMouseUp(buttonIndex);
        }
    }

    /** 发送鼠标按下事件，同时通知陀螺仪模块 */
    private void doMouseDown(byte button) {
        conn.sendMouseButtonDown(button);
        if (gyroAimManager != null) {
            gyroAimManager.onMouseButton(button, true);
        }
    }
    ...
    private void doMouseUp(byte button) {
        ...
        if (gyroAimManager != null) {
            gyroAimManager.onMouseButton(button, false);
        }
    }
    ...
    public void onPerfUpdate(final String text) {
        if ("performance".equals(BuildConfig.BUILD_TYPE)) {
            // Keep benchmark data machine-readable without adding any work when the overlay is off.
            LimeLog.info("PERF_STATS " + text.replace('\n', '|'));
        }
    ...
    // ===== TouchGamepadOverlay.InputInjector 实现 =====
    @Override
    public void sendKeyDown(short keyCode) {
        if (conn != null && connected) {
    ...
    @Override
    public void sendKeyUp(short keyCode) {
        if (conn != null && connected) {
