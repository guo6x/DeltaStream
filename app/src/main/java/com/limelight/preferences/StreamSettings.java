...
import com.limelight.PrivacyPolicyActivity;
...
public class StreamSettings extends Activity {
    ...
    public static class SettingsFragment extends PreferenceFragment {
        ...
        private int getRecommendedLowLatencyFps() {
            Display display = getActivity().getWindowManager().getDefaultDisplay();
            float maxSupportedFps = display.getRefreshRate();

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                for (Display.Mode mode : display.getSupportedModes()) {
                    maxSupportedFps = Math.max(maxSupportedFps, mode.getRefreshRate());
                }
            }

            // Cap the automatic profile at 120 FPS. Requesting above the host display/capture
            // rate adds duplicate pacing and bitrate pressure without producing fresher frames.
            // Leave a little tolerance for panels that report 119.88 Hz or 89.9 Hz.
            if (maxSupportedFps >= 118) {
                return 120;
            }
            else if (maxSupportedFps >= 88) {
                return 90;
            }
            else {
                return 60;
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            // ===== 一键应用低延迟推荐配置（USB 连接优化） =====
            final String optimizedResolution = PreferenceConfiguration.DEFAULT_RESOLUTION;
            final int optimizedFps = getRecommendedLowLatencyFps();
            final int optimizedBitrate = PreferenceConfiguration.getDefaultBitrate(
                    optimizedResolution, Integer.toString(optimizedFps));
            Preference optimizePref = new Preference(getActivity());
            optimizePref.setTitle("⚡ 一键应用低延迟推荐配置");
            optimizePref.setSummary("按本机屏幕自适应：1080p / " + optimizedFps +
                    "fps / HEVC / " + (optimizedBitrate / 1000) + "Mbps / 最低延迟帧节奏");
            optimizePref.setOnPreferenceClickListener(preference -> {
                SharedPreferences.Editor ed = PreferenceManager.getDefaultSharedPreferences(getActivity()).edit();
                ed.putString(PreferenceConfiguration.RESOLUTION_PREF_STRING, optimizedResolution);
                ed.putString(PreferenceConfiguration.FPS_PREF_STRING, Integer.toString(optimizedFps));
                ed.putString("frame_pacing", "latency");
                ed.putString("video_format", "force_hevc");
                ed.putBoolean("checkbox_low_latency_mode", true);
                ed.putBoolean("checkbox_enable_hdr", false);
                ed.putBoolean("checkbox_full_range", false);
                ed.putBoolean("checkbox_enable_audiofx", false);
                ed.putString("list_audio_config", "2");
                ed.putBoolean("checkbox_host_audio", false);
                // Scale bitrate with the chosen frame rate so latency doesn't trade away image quality.
                ed.putInt(PreferenceConfiguration.BITRATE_PREF_STRING, optimizedBitrate);
                ed.apply();
                android.widget.Toast.makeText(getActivity(),
                        "已应用低延迟推荐配置（1080p/" + optimizedFps + "fps/HEVC/" +
                                (optimizedBitrate / 1000) + "Mbps）\n请重新连接串流生效",
                        android.widget.Toast.LENGTH_LONG).show();
                // 刷新设置页显示
                ((StreamSettings) getActivity()).reloadSettings();
                return true;
            });
            PreferenceCategory basicCategory = (PreferenceCategory) findPreference("category_basic_settings");
            basicCategory.addPreference(optimizePref);
            ...
            // 隐私政策入口
            Preference privacyPref = findPreference("pref_privacy_policy");
            if (privacyPref != null) {
